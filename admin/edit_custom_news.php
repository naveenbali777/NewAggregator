...
if(isset($_POST["update"])) {
	...
	$result = $fun->updateArticle(article_table,$articleData,$id);
	if($result == "updated"){
...
$dataArray = $fun->getcustomsinglenews($id);
// echo "<pre>";
// print_r($dataArray[0]);
// exit;
?>
  ...
  <div class="row content">
    ...
    <div class="col-sm-9">
      <div class="row BoxOuter">
		<h2 class="heading">Edit Custom News</h2>
		<div><?php echo $errormsg; ?></div>
		<form method="post" enctype="multipart/form-data"> 
			<div class="form-group">
				<input type="text" class="form-control" name="article_title" placeholder="Article Title" value="<?php echo (!empty($dataArray[0]['article_title']) ? $dataArray[0]['article_title'] : ""); ?>" required>
			</div>
			<div class="form-group">
				<input type="text" class="form-control" name="article_author" placeholder="Article Author" value="<?php echo (!empty($dataArray[0]['article_author']) ? $dataArray[0]['article_author'] : ""); ?>">
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				<select name="category_id" class="form-control" required>
					...
					<?php foreach ($categoryArray as $key => $value) { ?>
						<option value="<?php echo $value['id']; ?>" <?php echo ($dataArray[0]['category_id'] == $value['id']) ? "selected" : ""; ?> ><?php echo $value['name']; ?></option>
					<?php } ?>
				</select>
			</div>
			<div class="form-group">
				<textarea placeholder="Site Note" class="form-control" name="article_txt" id="mytextarea"><?php echo (!empty($dataArray[0]['article_txt']) ? $dataArray[0]['article_txt'] : ""); ?></textarea>
			</div>
