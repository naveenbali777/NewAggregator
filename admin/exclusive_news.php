...
$condition = ($category_id != 0) ? " where ".article_table.".exclusive = 'yes' and ".article_table.".category_id = ".$category_id : " where ".article_table.".exclusive = 'yes'";
if($website_id != 0){
...
$condition = (!empty($condition)) ? $condition." and ".article_table.".exclusive = 'yes'" : $condition." where ".article_table.".exclusive = 'yes'";
...
$categoryArray = $fun->getcategorygroupby(" where ".article_table.".exclusive = 'yes'");
$websiteArray = $fun->getwebsitesgroupby(" where ".article_table.".exclusive = 'yes'");

if(isset($_POST['unexclusive'])){
	if(isset($_POST['articles'])){
		$con = $fun->conOpen();
	    foreach ($_POST['articles'] as $key => $value) {
			mysqli_query($con, "UPDATE ".article_table." SET `exclusive` = 'no' WHERE `id` = $value");
		}
	    mysqli_close($con);
  ...
  <div class="row content">
    ...
    <div class="col-sm-9">
      <div class="row BoxOuter">
		<div class="col-sm-12">
			<h2 class="heading">Exclusive News</h2>
		</div>
		<div class="col-sm-12">
			<form method="get">
				<div class="row">
		...
		<div class="col-sm-12">
		<?php if($count > 1){ ?>
			<input type="submit" name="unexclusive" value="Unexclusive">			
		<?php } ?>
		</div>
		</form>
		<div class="col-sm-12">
