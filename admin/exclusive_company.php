...
$fun = new FetchData();
$fun->authentication();
$page_name = "exclusive_company.php";
...
$condition = ($serchTxt != "") ? " where company.exclusive = 'yes' and company.".$fieldName." LIKE "."'%$serchTxt%'" : " where company.exclusive = 'yes'";
$totalCount = $fun->recordCount('company', $condition);
$condition = $condition." order by company.last_trade_price desc limit $start,$index";
...
if(isset($_POST['unexclusive'])){
	if(isset($_POST['articles'])){
	    ...
	    foreach ($_POST['articles'] as $key => $value) {
	    	mysqli_query($con, "UPDATE company SET `exclusive` = 'no' WHERE `id` = $value");
		}
	    mysqli_close($con);
  ...
  <div class="row content">
    ...
    <div class="col-sm-9">
      <div class="row BoxOuter">
		<h2 class="heading">Exclusive Companies</h2>
		<div class="col-sm-12">
			<form method="get">
				<div class="row">
					<div class="col-sm-3">
