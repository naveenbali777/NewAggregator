...
class FetchData
{
	...
	function getwebsites($condition=""){
		$dataArray = array();
		$con = $this->conOpen();
		$sql = "SELECT websites.`id`,websites.`name`,websites.`url`,websites.`alexa_rank`,websites.`active`,websites.`note`,websites.`tags`,category.`name` as category_name FROM websites INNER JOIN category ON websites.`category_id` = category.`id`".$condition;
		$query = mysqli_query($con, $sql);
		while ($row = mysqli_fetch_assoc($query)) {
			$dataArray[] = $row;
		}
	...
	function insert_website($data){
		...
		$sdata = mysqli_fetch_assoc($querys);
		if(empty($sdata)){
			mysqli_query($con, "INSERT INTO websites(`category_id`, `name`, `url`, `alexa_rank`, `suggested`, `note`, `created_at`, `updated_at`) VALUES('".$category_id."', '".$name."', '".$url."', '".$alexa_rank."', '".$suggested."', '".$note."', '".$date."', '".$date."') ");
			mysqli_close($con);
			return "insert";
		}else{
			mysqli_close($con);
